...
app.add_middleware(
    ...
    allow_origins=os.getenv(
        ...
        ","
    ),  # Seguridad Estricta: Solo permitir el frontend oficial
...
@app.post(
    "/children",
    response_model=schemas.ChildResponse,
    status_code=status.HTTP_201_CREATED,
)
def create_child(
    ...
    current_user: models.User = Depends(
        auth.RoleChecker(["ADMIN", "PROFESSIONAL", "CAREGIVER"])
    ),
):
...
def list_children(
    ...
    if current_user.role == "ADMIN" or current_user.role == "PROFESSIONAL":
        ...
        return (
            ...
            .join(models.AgentChild)
            .filter(models.AgentChild.agent_id == agent.id)
            .all()
        )
        ...
        return (
            db.query(models.Child)
            .join(models.CaregiverChild)
            .filter(models.CaregiverChild.caregiver_id == caregiver.id)
            .all()
        )
...
def get_child(
    ...
    if not child:
        raise HTTPException(
            status_code=404, detail="El niño no se encuentra registrado."
        )

    # Check permissions
    if current_user.role == "CAREGIVER":
        link = (
            db.query(models.CaregiverChild)
            .filter(
        ...
        if not link:
            raise HTTPException(
                status_code=403, detail="No tienes autorización para ver esta ficha."
            )

    elif current_user.role == "COMMUNITY_AGENT":
        link = (
            db.query(models.AgentChild)
            .filter(
        ...
        if not link:
            raise HTTPException(
                status_code=403, detail="No tienes asignado este niño para seguimiento."
            )
...
@app.post(
    "/children/{child_id}/measurements", response_model=schemas.MeasurementResponse
)
def add_measurement(
    child_id: int,
    ...
    if current_user.role == "CAREGIVER":
        ...
        if not link:
            raise HTTPException(
                status_code=403, detail="Permiso denegado para registrar mediciones."
            )
    ...
    last_measure = (
        ...
        .order_split
    ) = models.Measurement.measurement_date.desc()
    ...
    last_measure = (
        ...
        .order_by(models.Measurement.measurement_date.desc())
        .first()
    )
    ...
    rule_ver = (
        ...
        .filter(models.RuleVersion.rule_id == rule.id)
        .first()
    )
    if not rule_ver:
        rule_ver = models.RuleVersion(
            rule_id=rule.id, version="1.0", source_doc=rule_source
        )
        db.add(rule_ver)
        db.commit()
    ...
    log_action(
        db,
        current_user.id,
        f"register_measurement_{measure_in.type}",
        "measurements",
        measurement.id,
    )
    return measurement
...
def get_dashboard_summary(
    ...
    normal = (
        db.query(models.Child).filter(models.Child.status_alerta == "normal").count()
    )
    follow = (
        db.query(models.Child).filter(models.Child.status_alerta == "follow-up").count()
    )
    urgent = (
        db.query(models.Child).filter(models.Child.status_alerta == "urgent").count()
    )
...
def get_dashboard_prioritized(
    ...
    for c in children:
        ...
        w_measure = (
            db.query(models.Measurement)
            .filter(
                models.Measurement.child_id == c.id, models.Measurement.type == "weight"
            )
            .order_by(models.Measurement.measurement_date.desc())
            .first()
        )
        ...
        last_measured = (
            w_measure.measurement_date.strftime("%d/%m/%Y")
            if w_measure
            else "Sin registro"
        )
        ...
        h_measure = (
            db.query(models.Measurement)
            .filter(
                models.Measurement.child_id == c.id, models.Measurement.type == "height"
            )
            .order_by(models.Measurement.measurement_date.desc())
            .first()
        )
        ...
        m_measure = (
            db.query(models.Measurement)
            .filter(
                models.Measurement.child_id == c.id, models.Measurement.type == "muac"
            )
            .order_by(models.Measurement.measurement_date.desc())
            .first()
        )
        ...
        cg_link = (
            db.query(models.CaregiverChild)
            .filter(models.CaregiverChild.child_id == c.id)
            .first()
        )
        caregiver_name = (
            f"{cg_link.caregiver.name} {cg_link.caregiver.lastname}"
            if cg_link
            else "Desconocido"
        )
...
def record_visit(
    ...
    if not agent:
        raise HTTPException(
            status_code=400,
            detail="Este usuario no cuenta con un perfil de Actor Social.",
        )
...
def chat_with_ai(
    ...
    if not api_key:
        raise HTTPException(
            status_code=503,
            detail="AI Service offline (No API Key). Fallback to local NLU.",
        )
    ...
    try:
        ...
        print(f"LLM Error: {e}")
        raise HTTPException(
            status_code=503, detail="AI Service offline. Fallback to local NLU."
        )
...
@app.patch(
    "/admin/users/{user_id}/status", response_model=schemas.UserResponse, tags=["Admin"]
)
def update_user_status(
    ...
    if user.role == "ADMIN":
        raise HTTPException(
            status_code=400,
            detail="No se puede modificar el estado de un administrador.",
        )
    ...
    db.refresh(user)
    log_action(
        db, current_user.id, f"set_user_status_{status_in.status}", "users", user_id
    )
    return user
...
def get_admin_stats(
    ...
    return schemas.AdminStats(
        ...
        .filter(models.Alert.status_revision == "pending")
        .count(),
        visits_this_month=db.query(models.Visit)
        .filter(models.Visit.visit_date >= month_start)
        .count(),
        caregivers=db.query(models.User)
        .filter(models.User.role == "CAREGIVER", models.User.status == "active")
        .count(),
        professionals=db.query(models.User)
        .filter(models.User.role == "PROFESSIONAL", models.User.status == "active")
        .count(),
        community_agents=db.query(models.User)
        .filter(models.User.role == "COMMUNITY_AGENT", models.User.status == "active")
        .count(),
        normal_children=db.query(models.Child)
        .filter(models.Child.status_alerta == "normal")
        .count(),
        follow_up_children=db.query(models.Child)
        .filter(models.Child.status_alerta == "follow-up")
        .count(),
        urgent_children=db.query(models.Child)
        .filter(models.Child.status_alerta == "urgent")
        .count(),
    )
...
def assign_professional_to_child(
    ...
    existing = (
        db.query(models.ProfessionalChild)
        .filter(
    ...
    db.commit()
    log_action(
        db, current_user.id, "assign_professional_child", "professional_child", child_id
    )
    return {
        "message": "Asignación creada correctamente.",
