...
def test_auth_flow(client):
    ...
    resp_login = client.post(
        "/auth/login", data={"username": "mariapru", "password": "mariapassword"}
    )
...
def test_child_and_measurement(client):
    ...
    login_resp = client.post(
        "/auth/login", data={"username": "carlospru", "password": "carlospassword"}
    )
    ...
    measure_resp = client.post(
        ...
        headers=headers,
        json={"type": "weight", "value": 9.0, "unit": "kg", "method": "professional"},
    )
