...
class User(Base):
    ...
    )  # ADMIN, PROFESSIONAL, COMMUNITY_AGENT, CAREGIVER
    ...
    professional = relationship(
        "HealthProfessional", uselist=False, back_populates="user"
    )
...
class Child(Base):
    ...
    # Relationships
    caregivers_links = relationship(
        "CaregiverChild", back_populates="child", cascade="all, delete-orphan"
    )
    agents_links = relationship(
        "AgentChild", back_populates="child", cascade="all, delete-orphan"
    )
    professionals_links = relationship(
        "ProfessionalChild", back_populates="child", cascade="all, delete-orphan"
    )
    measurements = relationship(
        "Measurement", back_populates="child", cascade="all, delete-orphan"
    )
    ...
    nutrition_guidance = relationship(
        "NutritionGuidance", back_populates="child", cascade="all, delete-orphan"
    )
...
class Measurement(Base):
    ...
    validation = relationship(
        "MeasurementValidation", uselist=False, back_populates="measurement"
    )
...
class AuditLog(Base):
    ...
    )  # None for anonymous actions
    action = Column(
        String, nullable=False
    )  # login, create_child, register_measurement, etc.
