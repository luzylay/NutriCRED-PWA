...
def seed_db():
    ...
    try:
        ...
        admin = models.User(
            username="admin",
            email_or_phone="admin@yanapiri.org",
        ...
        prof_user = models.User(
            username="carlos",
            email_or_phone="carlos.mendoza@diresahvca.gob.pe",
            password_hash=prof_pwd,
            role="PROFESSIONAL",
        )
        db.add(prof_user)
        db.commit()
        ...
        agent_user = models.User(
            username="luisa",
            email_or_phone="966554433",
            password_hash=agent_pwd,
        ...
        c1 = models.Child(
            ...
            - datetime.timedelta(days=37 * 30.4),
            district="Huancavelica",
            community="Ccasapata",
            zscore_actual=-2.8,
            status_alerta="urgent",
        )
        c2 = models.Child(
            name="Rosa Huanca Pérez",
            sex="F",
            date_of_birth=datetime.datetime.utcnow()
            - datetime.timedelta(days=16 * 30.4),
            district="Huancavelica",
            community="Lircay",
            zscore_actual=-2.5,
            status_alerta="urgent",
        ...
        c3 = models.Child(
            ...
            sex="M",
            date_of_birth=datetime.datetime.utcnow()
            - datetime.timedelta(days=27 * 30.4),
            district="Huancavelica",
            community="Anchonga",
            zscore_actual=-1.8,
        ...
        c4 = models.Child(
            ...
            sex="M",
            date_of_birth=datetime.datetime.utcnow()
            - datetime.timedelta(days=33 * 30.4),
            district="Huancavelica",
            community="Ccasapata",
            zscore_actual=-1.4,
            status_alerta="follow-up",
        ...
        c5 = models.Child(
            ...
            sex="F",
            date_of_birth=datetime.datetime.utcnow()
            - datetime.timedelta(days=20 * 30.4),
            district="Huancavelica",
            community="Lircay",
            zscore_actual=-0.4,
        ...
        c6 = models.Child(
            ...
            sex="F",
            date_of_birth=datetime.datetime.utcnow()
            - datetime.timedelta(days=11 * 30.4),
            district="Huancavelica",
            community="Anchonga",
            zscore_actual=0.2,
            status_alerta="normal",
        ...
        m_juan = [
            models.Measurement(
                child_id=c3.id,
                registered_by=cg_user.id,
                type="weight",
                value=10.2,
                unit="kg",
                measurement_date=datetime.datetime.utcnow()
                - datetime.timedelta(days=9 * 30),
                method="self",
            ),
            models.Measurement(
                child_id=c3.id,
        ...
        for m in db.query(models.Measurement).all():
            v = models.MeasurementValidation(
                measurement_id=m.id,
                is_valid=True,
                validation_notes="Rango inicial.",
        ...
        a1 = models.Alert(
            child_id=c1.id,
            measurement_id=m_pedro[0].id,
            rule_version_id=ver1.id,
            level="urgent",
            created_at=datetime.datetime.utcnow() - datetime.timedelta(days=12),
            comments="Desviación crítica de peso-para-edad (Z-score = -2.8).",
        )
        a1_muac = models.Alert(
            child_id=c1.id,
            measurement_id=m_pedro[2].id,
            rule_version_id=ver2.id,
            level="urgent",
            created_at=datetime.datetime.utcnow() - datetime.timedelta(days=12),
            comments="Perímetro braquial (MUAC) crítico de 11.5 cm.",
        )

        # Rosa Z-score = -2.5 (urgent)
        a2 = models.Alert(
            child_id=c2.id,
            measurement_id=m_rosa[0].id,
            rule_version_id=ver1.id,
            level="urgent",
            created_at=datetime.datetime.utcnow() - datetime.timedelta(days=1),
            comments="Desviación crítica de peso-para-edad (Z-score = -2.5).",
        )

        # Juan Z-score = -1.8 (follow-up)
        a3 = models.Alert(
            child_id=c3.id,
            measurement_id=m_juan[4].id,
            rule_version_id=ver1.id,
            level="follow-up",
            created_at=datetime.datetime.utcnow() - datetime.timedelta(days=3),
            comments="Pérdida moderada de peso detectada (Z-score = -1.8).",
        )
        ...
        v1 = models.Visit(
            ...
            visit_date=datetime.datetime.utcnow() - datetime.timedelta(days=5),
            visit_type="Control de Alerta",
        )
        db.add(v1)
        db.commit()
        ...
        obs1 = models.Observation(
            visit_id=v1.id,
            qualitative_notes="Se coordinó con la madre. Juan se encuentra con apetito pero la madre refiere no contar con suplemento de hierro desde hace dos semanas.",
            alarm_signs="Ninguno",
        )
        db.add(obs1)
