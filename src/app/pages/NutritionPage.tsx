...
const RECIPES = [
  {
    ...
    iron: "Alto",
    description:
      "Ideal para iniciar la alimentación complementaria y combatir la anemia. La sangrecita es el alimento con más hierro.",
    ingredients: [
      "2 cucharadas de sangrecita de pollo cocida",
  ...
  },
  {
    ...
    iron: "Medio",
    description:
      "La quinua aporta proteínas completas y la manzana facilita la digestión. Perfecto para la media tarde.",
    ingredients: [
      "3 cucharadas de quinua lavada",
      "1/2 manzana delicia",
      "Canela al gusto (sin azúcar)",
    ],
    time: "20 min",
  },
  {
    ...
    iron: "Medio",
    description:
      "Fuente excelente de hierro no hemo y fibra. Acompañar siempre con limonada.",
    ingredients: [
...
const SUPERFOODS = [
  {
    ...
    icon: "🌱",
    naturist:
      "Considerada por los Incas como 'la semilla sagrada' que da fuerza y calienta el cuerpo en las alturas.",
    scientific:
      "Contiene hasta un 19% de proteína (superior a la quinua). Rica en Hierro (15mg/100g) y antioxidantes (quercetina).",
  ...
  },
  {
    ...
    icon: "🌻",
    naturist:
      "Usado históricamente para limpiar el cuerpo y fortalecer los huesos por los pueblos originarios.",
    scientific:
      "Leguminosa con más de 40% de proteína de alto valor y gran cantidad de Calcio (hasta 115mg/100g).",
  ...
  },
  {
    ...
    icon: "🍒",
    naturist:
      "Fruta selvática usada para curar resfríos, dar vitalidad y proteger contra enfermedades del pulmón.",
    scientific:
      "Tiene la mayor concentración de Vitamina C del mundo (hasta 3000mg/100g), multiplicando exponencialmente la absorción del hierro vegetal.",
    benefit:
      "Potenciador del hierro (clave contra la anemia) y refuerzo del sistema inmune.",
  },
  {
    id: "sf4",
    name: "Sangrecita",
    ...
    icon: "🩸",
    naturist:
      "Remedio casero tradicional de las abuelas para 'curar el susto' y levantar a niños débiles o pálidos.",
    scientific:
      "Fuente suprema de Hierro Hemo (el cuerpo humano lo absorbe al 30%, vs 5% de los vegetales). Trata la anemia ferropénica clínicamente.",
    benefit:
      "El alimento más rápido para sacar a un niño de la desnutrición crónica.",
  },
];
...
const NUTRIENTS_DB = {
  primary: {
    ...
    deficiency:
      "Su falta causa desnutrición crónica (stunting), retraso en el crecimiento y debilidad muscular.",
  ...
  secondary: {
    ...
    deficiency:
      "Su falta puede causar huesos frágiles, problemas dentales y calambres.",
    foods:
      "Encuéntralos en la Leche, Tarwi, Quesos y vegetales de hojas muy verdes.",
  },
  oligo: {
    ...
    deficiency:
      "Su falta causa ANEMIA, cansancio excesivo, palidez y daño irreversible al desarrollo intelectual.",
    foods:
      "Encuéntralos en abundancia en la Sangrecita, Hígado, Bazo y pescados oscuros.",
  },
};
...
export default function NutritionPage() {
  ...
  const filteredRecipes =
    selectedAge === "Todas"
      ? RECIPES
      : RECIPES.filter((r) => r.age === selectedAge);
        ...
        <div className="flex justify-between items-start relative z-10">
          <div>
            ...
            <p className="text-white/80 mt-2 text-sm max-w-[250px] leading-relaxed">
              Recetas peruanas ricas en hierro para que tu wawa crezca fuerte y
              sin anemia.
            </p>
          </div>
      ...
      <div className="px-5 -mt-8 relative z-20 space-y-6">
        ...
        <div className="bg-card rounded-[2rem] p-5 shadow-lg border border-border flex gap-4 items-center">
          ...
          <div>
            <h3 className="font-bold text-foreground">
              Escudo contra la Anemia
            </h3>
            <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
              Recuerda acompañar las comidas ricas en hierro con vitamina C
              (limonada, naranja) para que el cuerpo lo absorba mejor.
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-4 border-b border-border mb-6">
          ...
          >
            <span className="flex items-center gap-2">
              <ChefHat className="size-4" /> Recetario Local
            </span>
          </button>
          <button
          ...
          >
            <span className="flex items-center gap-2">
              <Library className="size-4" /> La Despensa
            </span>
          </button>
        </div>
          ...
          <>
            ...
            <div>
              <div className="flex gap-2 overflow-x-auto hide-scrollbar pb-2">
                {["Todas", "6 a 8 meses", "9 a 11 meses", "12 a 24 meses"].map(
                  (age) => (
                    <button
                      key={age}
          ...
          <div className="space-y-6">
            <div className="bg-gradient-to-r from-emerald-500/10 to-teal-500/10 border border-emerald-200 dark:border-emerald-900/50 rounded-3xl p-5 mb-4">
              <h3 className="font-bold text-foreground flex items-center gap-2 mb-2">
                ...
                Catálogo de Súper-Alimentos
              </h3>
              <p className="text-sm text-muted-foreground leading-relaxed">
                Nuestra tierra nos provee. Descubre el balance perfecto entre el
                conocimiento ancestral y la evidencia científica (OMS / INS)
                sobre los tesoros nutricionales de Perú y el mundo.
              </p>
            </div>
            ...
            <div className="bg-muted/30 border border-border p-4 rounded-3xl mb-6">
              <h4 className="text-sm font-bold text-foreground mb-3 flex items-center gap-2">
                <Info className="size-4 text-blue-500" /> Glosario: Nutrientes
                Esenciales
              </h4>
              <p className="text-xs text-muted-foreground leading-relaxed mb-4">
                Toca cualquier nutriente para entender profundamente por qué es
                vital para tu bebé, sin salir de la aplicación.
              </p>

              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <button
                  onClick={() => setSelectedNutrient("primary")}
                  className="bg-card border border-border p-3 rounded-2xl flex items-center justify-between hover:border-blue-300 hover:shadow-md transition-all text-left"
                >
                  <span className="text-xs font-bold text-foreground">
                    1. Primarios (Proteínas)
                  </span>
                  <span className="size-6 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 text-xs">
                    →
                  </span>
                </button>
                <button
                  onClick={() => setSelectedNutrient("secondary")}
                  className="bg-card border border-border p-3 rounded-2xl flex items-center justify-between hover:border-amber-300 hover:shadow-md transition-all text-left"
                >
                  <span className="text-xs font-bold text-foreground">
                    2. Secundarios (Calcio)
                  </span>
                  <span className="size-6 bg-amber-100 rounded-full flex items-center justify-center text-amber-600 text-xs">
                    →
                  </span>
                </button>
                <button
                  onClick={() => setSelectedNutrient("oligo")}
                  className="bg-card border border-border p-3 rounded-2xl flex items-center justify-between hover:border-red-300 hover:shadow-md transition-all text-left"
                >
                  <span className="text-xs font-bold text-foreground">
                    3. Oligoelementos (Hierro)
                  </span>
                  <span className="size-6 bg-red-100 rounded-full flex items-center justify-center text-red-600 text-xs">
                    →
                  </span>
                </button>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {SUPERFOODS.map((food) => (
                <div
                  key={food.id}
                  className="bg-card border border-border rounded-[2rem] overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col"
                >
        ...
        <div className="mt-8 border-t border-border pt-6 pb-4 text-center">
          ...
          <p className="text-xs text-muted-foreground max-w-md mx-auto leading-relaxed">
            Las recetas sugeridas están basadas en los lineamientos públicos del{" "}
            <strong>Instituto Nacional de Salud (INS)</strong> y el{" "}
            <strong>Ministerio de Salud del Perú (MINSA)</strong>. <br />
            <br />
            <em>Descargo de responsabilidad:</em> Esta información es de
            carácter educativo y preventivo. No sustituye el diagnóstico,
            consejo o tratamiento de un pediatra o nutricionista certificado.
          </p>
        </div>
      </div>
      ...
      {selectedNutrient && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
          <div className="bg-card w-full max-w-md rounded-[2rem] shadow-2xl border border-border overflow-hidden animate-in fade-in zoom-in duration-200">
            <div
              className={`${NUTRIENTS_DB[selectedNutrient].color} p-6 text-white relative`}
            >
              <button
              ...
              <h3 className="text-xl font-black">
                {NUTRIENTS_DB[selectedNutrient].title}
              </h3>
            </div>
            ...
            <div className="p-6 space-y-5">
              <div>
                <h4 className="text-xs uppercase font-bold text-muted-foreground mb-1">
                  ¿Qué son?
                </h4>
                <p className="text-sm text-foreground leading-relaxed">
                  {NUTRIENTS_DB[selectedNutrient].desc}
                </p>
              </div>

              <div className="bg-red-50/50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/30 p-4 rounded-2xl">
                <h4 className="text-xs uppercase font-bold text-red-600 dark:text-red-400 mb-1">
                  ⚠️ ¿Qué pasa si le falta?
                </h4>
                <p className="text-sm text-foreground leading-relaxed">
                  {NUTRIENTS_DB[selectedNutrient].deficiency}
                </p>
              </div>

              <div>
                <h4 className="text-xs uppercase font-bold text-green-600 dark:text-green-500 mb-1">
                  ¿Dónde los encuentro?
                </h4>
                <p className="text-sm text-foreground leading-relaxed font-medium">
                  {NUTRIENTS_DB[selectedNutrient].foods}
                </p>
              </div>

              <button
                onClick={() => setSelectedNutrient(null)}
                className="w-full bg-muted hover:bg-muted/80 text-foreground font-bold py-3 rounded-full transition-colors mt-2 text-sm"
              >
