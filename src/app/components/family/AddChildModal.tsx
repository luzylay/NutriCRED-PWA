      ...
      <div className="w-full max-w-md bg-card rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-300">
        ...
        <div className="bg-primary/10 border-b border-border px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-foreground font-nunito">Registrar Niño/a</h2>
              <p className="text-xs text-muted-foreground">Agrega un nuevo miembro a tu familia</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-4">
              ...
              <div className="flex bg-muted/50 rounded-xl border border-border p-1">
                <button
                  ...
                  className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${
                    formData.sex === "M" ? "bg-white shadow-sm text-primary" : "text-muted-foreground"
                  }`}
                >
                  Niño
                </button>
                <button
                  ...
                  className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${
                    formData.sex === "F" ? "bg-white shadow-sm text-primary" : "text-muted-foreground"
                  }`}
                >
                  Niña
                </button>
          ...
          <div className="bg-accent/10 border border-accent/20 rounded-2xl p-4 space-y-4">
            <p className="text-xs font-bold text-accent uppercase tracking-wider">Medición Inicial (Opcional)</p>
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
